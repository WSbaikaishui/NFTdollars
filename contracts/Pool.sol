...
import "./interfaces/INFTUSDToken";
import "./interfaces/INFTOracle";
import "./interfaces/INDLToken";
  ...
  Ownable
{
  ...
  mapping (address => Snapshots) public depositSnapshots;  // depositor address -> snapshots struct
    ...
    uint256 public totalNFTUSDDeposits;
   ...
   **/
  function initialize(address nFTOracle, address nftUSDToken, address nDLToken, address poolLoans,address community) public initializer {
    nftusdToken = INFTUSDToken(nftUSDToken);
    ndlToken = INDLToken(nDLToken);
    nftOracle = INFTOracle(nFTOracle);
    poolLoan = IPoolLoan(poolLoans);
    communityIssuance = ICommunityIssuance(community);
    percentBorrow = 9*1000;//percent 90%;
    borrowFee = 400; //percent 4%
    _renounceOwnership();
  }

  function getTotalNFTUSDDeposits() external view override returns (uint) {
  ...
  function deposit(uint _amount) external override {
    ...
    */
    function getDepositorNFTUSDGain(address _depositor) public view override returns (uint) {
        ...
        uint NFTUSDGain = _getNFTUSDGainFromSnapshots(initialDeposit, snapshots);
        return NFTUSDGain;
    }
    ...
    function _getNFTUSDGainFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        ...
        */
        ...
        uint NTFTUSDGain = initialDeposit.mul(firstPortion.add(secondPortion)).div(P_Snapshot).div(DECIMAL_PRECISION);
    ...
    function _sendNFTUSDGainToDepositor(ICommunityIssuance _communityIssuance,address _depositor,uint _amount) internal {
        if (_amount == 0) {return;}
        uint newNFTUSD = NFTUSD.sub(_amount);
        NFTUSD = newNFTUSD;
        emit StabilityPoolNFTUSDBalanceUpdated(newNFTUSD);

        _communityIssuance.sendNFTUSD(_depositor,_amount);
        emit LQTYPaidToDepositor(_depositor, _amount);
    }
    ...
    function getDepositorNDLGain(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {return 0;}
        /*
     ...
     function _payOutNDLGains(ICommunityIssuance _communityIssuance, address _depositor) internal {
  ...
  function borrow(
   ...
   require(nftfloorPrice != 0,"no such nft supplyed")
    ...
    require(maxDebtPrice > amount,"borrow amout is to high for this NFT");

    if (loanId == 0) {
      loanId = poolLoan.createLoan(
        initiator,
        onBehalfOf,
        nftAsset,
        nftTokenId,
    ...
    nftusdToken.mint(
      initiator,
      onBehalfOf,
      amount.percentMul(percentBorrow),//TODO 定义percentBorrow
    );
    nftusdToken.mint(
      address(this),
      onBehalfOf,
     amount - amount.percentMul(percentBorrow),//TODO 定义percentBorrow
    );
    ndlToken.transferFrom(initiator,address(this), amount.percentMul(borrowFee));
    emit Borrow(
    initiator,
      asset,
      amount,
  ...
  function liquidate(
    ...
    uint newtotalSecurityDeposit = totalSecurityDeposit - loan.amount.percentMul(1e4-percentBorrow);
    if (!success){
